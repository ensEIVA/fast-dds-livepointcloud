...
ipcMain.handle('subscribe', async (event) => {
  if (worker) {
    console.log('Worker already started...');
    return;
  }
  worker = fork('pointCloudChild.js', ['pointCloudChild']);//fork(resolve(__dirname, 'pointCloudChild.js'), ['pointCloudChild']);
...
ipcMain.handle('unsubscribe', async (event) => {
  if (!worker) {
